...
import { imageService, cervixAnalysisService, patientService } from "@/lib/api-services"
...
export default function CervixAnalysis() {
  ...
  const handleAnalysis = async () => {
    ...
    try {
      // ✅ Targeted query — fetch only the matching patient document
      ...
      console.log("[v0] Analysis saved:", analysisId)
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {new Date(analysis.createdAt?.toDate ? analysis.createdAt.toDate() : analysis.createdAt || Date.now()).toLocaleDateString()} at{" "}
                        {new Date(analysis.createdAt?.toDate ? analysis.createdAt.toDate() : analysis.createdAt || Date.now()).toLocaleTimeString()}
                      </p>
                    </div>
